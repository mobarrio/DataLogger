...
$SIG{'HUP'} = sub { Balloon( "Datalogger v1.1: Extrae informacion de un puerto serie y lo deja en una base de datos MySQL" ); };
...
RegisterHotKey("ALT+F2", \&start);
RegisterHotKey("ALT+F3", \&stop);
RegisterHotKey("ALT+SHIFT+x", "exit");
#SetTimer(":5", 'Balloon("DataLogger Biolac: [$status]")' );

#sub StartAnim
#{
#   my @anim = qw(icon3 icon4 icon5 icon6);
#   SetAnimation("0:20", 100, @anim);
#}
...
{ 
	...
	$status = "Online";
	SetIcon('icon2'); 
	Balloon("Datalogger is Online...",'','info', 3);
	$thr = threads->create('Main'); 
}
...
{ 
	...
	$status = "Offline";
	SetIcon('icon1');
	Balloon("Datalogger is Offline...",'','info', 3);
}

sub ToolTip { "DataLogger Biolac: [$status]" }
...
sub sendsig { $thr->kill('HUP'); }

sub Main 
{
	$serial  = openPort($DEVICE);
	print STDERR "DataLogger is ON [$logtime]\n" if(!$quiet);
	print STDERR "Open Serial Port : $DEVICE, $baudrate, $databits, $parity, $stopbits, $handshake\n"  if(!$quiet);
	...
	logInfo("Esperando Inicio de Transmision [ENQ].\n");
	...
	while($flag)
	{
		...
		{					
			...
			{							
				...
				logInfo("Esperando Inicio de Transmision [ENQ].\n");
				Balloon("Transferencia Finalizada, esperando Inicio de Transmision...",'','info', 3);
			}
			Balloon("Transferencia en progreso...",'','info', 3);
...
__HELP__
 ...
 Balloon( "Esquema $filename creado..." );
}
